#include <stdio.h>
#include <string.h>

#include "vcsoc.h"

static void crypto_test1(void);
static void crypto_test2(void);
static void crypto_test3(void);

static void dump(uint8_t *buf, uint32_t len);

//Note: set variable below to run specific tests
#define TEST1 1
#define TEST2 1
#define TEST3 1

int vcradio_lock_irq(void)
{
  return 0;
}

void vcradio_unlock_irq(int temp)
{
  (void) temp;
}

int main(void)
{
  vccrypt_init(0); // use dma loopback

  printf("Start Security Test:\n");

  crypto_test1();
  crypto_test2();
  crypto_test3();
 
  while(1);

  return 0;
}

static void crypto_test1(void)
{
#if TEST1
    uint8_t key[] = {
        0xa8, 0x79, 0xac, 0x89, 0x71, 0x8f, 0x34, 0x13, 0x2a, 0xe5, 0x4a, 0xdc, 0x13, 0xd7, 0x7d, 0x18
    };

    uint8_t test[] = {
        0x69, 0xee, 0xe6, 0xd2, 0x04, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x0e, 0x1b, 0x00, 0x00, 0x00,
        0x01, 0x05, 0x15, 0x01, 0x04, 0x0a, 0xed, 0x05,
        0x06, 0x15, 0x02, 0xd1, 0x5a, 0xef, 0x01, 0x00,
        0x00, 0x3f,

        0x0e, 0xa0, 0x0c, 0x88, 0xc8, 0x00, 0x00, 0x01,
        0xb8, 0x2c, 0x07, 0x10, 0x62, 0x00, 0x00, 0x00,
        0x3a, 0x98, 0x88, 0xed, 0xa0, 0x7e, 0x33, 0xf0,
        0x02, 0x22, 0x02, 0x23, 0x25, 0xad, 0x01, 0x6c,
        0x19, 0x6d, 0x00, 0x01, 0x00, 0x0c, 0x00, 0x03,
        0x00, 0x06, 0x00, 0x00, 0x00, 0xff, 0xfe, 0x00,
        0x01, 0x01, 0x00, 0x03, 0x00, 0x0c, 0xa8, 0x7b,
        0x0d, 0x29, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0x00, 0x08, 0x00, 0x02, 0x00, 0x00,
        0x00, 0x0e, 0x00, 0x00, 0x00, 0xf8
    };

    uint16_t a_len = 42, m_len = 78;
    uint8_t  mic_len = 8;

    uint8_t m[m_len];
    uint8_t a[a_len];

    memcpy(m, &test[a_len], m_len);
    memcpy(a, test, a_len);

    uint8_t mic_enc[mic_len];
    uint8_t mic_dec[mic_len];

    uint8_t nonce[] = {
        0x00, 0x00, 0x00, 0xff, 0xfe, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x1b, 0x06
    };

    uint8_t encrypted[] = {
        0x69, 0xee, 0xe6, 0xd2, 0x04, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x0e, 0x1b, 0x00, 0x00, 0x00,
        0x01, 0x05, 0x15, 0x01, 0x04, 0x0a, 0xed, 0x05,
        0x06, 0x15, 0x02, 0xd1, 0x5a, 0xef, 0x01, 0x00,
        0x00, 0x3f,

        0x08, 0x18, 0xff, 0x4b, 0x50, 0x1f, 0x30, 0x15,
        0x38, 0xd9, 0xc5, 0x22, 0x17, 0xb7, 0x08, 0x78,
        0xb3, 0xa0, 0x98, 0xf9, 0xce, 0x10, 0x63, 0x56,
        0x30, 0xef, 0xe1, 0x92, 0xe9, 0xa6, 0x0c, 0x1d,
        0x84, 0x26, 0xbe, 0x3b, 0x9b, 0xc3, 0xa3, 0x76,
        0xc6, 0x19, 0xe0, 0x27, 0x59, 0x11, 0x5a, 0xe6,
        0x72, 0xd9, 0x36, 0x23, 0xc8, 0xf6, 0xff, 0x02,
        0xeb, 0xbb, 0xe0, 0x19, 0x2e, 0x3e, 0xfb, 0x27,
        0xd7, 0x52, 0x57, 0xbd, 0xf0, 0x98, 0xc9, 0x70,
        0x45, 0xf4, 0xd0, 0x7e, 0x7e, 0x88
    };

    uint8_t decrypted[] = {
        0x69, 0xee, 0xe6, 0xd2, 0x04, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x0e, 0x1b, 0x00, 0x00, 0x00,
        0x01, 0x05, 0x15, 0x01, 0x04, 0x0a, 0xed, 0x05,
        0x06, 0x15, 0x02, 0xd1, 0x5a, 0xef, 0x01, 0x00,
        0x00, 0x3f,

        0x0e, 0xa0, 0x0c, 0x88, 0xc8, 0x00, 0x00, 0x01,
        0xb8, 0x2c, 0x07, 0x10, 0x62, 0x00, 0x00, 0x00,
        0x3a, 0x98, 0x88, 0xed, 0xa0, 0x7e, 0x33, 0xf0,
        0x02, 0x22, 0x02, 0x23, 0x25, 0xad, 0x01, 0x6c,
        0x19, 0x6d, 0x00, 0x01, 0x00, 0x0c, 0x00, 0x03,
        0x00, 0x06, 0x00, 0x00, 0x00, 0xff, 0xfe, 0x00,
        0x01, 0x01, 0x00, 0x03, 0x00, 0x0c, 0xa8, 0x7b,
        0x0d, 0x29, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0x00, 0x08, 0x00, 0x02, 0x00, 0x00,
        0x00, 0x0e, 0x00, 0x00, 0x00, 0xf8
    };

    printf("------------------------------------------------------\n");
    printf("crypto_test1: encrypt & decrypt [%u]\n", m_len);
    printf("------------------------------------------------------\n");

    vccrypt_aes_ccm_encrypt(1, key, nonce, m, m_len, a, a_len, mic_enc, mic_len);

    if (memcmp(m, &encrypted[a_len], m_len) == 0) {
        printf("encrypted: succeed\n");
    } else {
        printf("encrypted: failed\n");
        while (1);
    }

    //dump(m, m_len);

    printf("mic_enc:\n");
    dump(mic_enc, mic_len);

    vccrypt_aes_ccm_encrypt(0, key, nonce, m, m_len, a, a_len, mic_dec, mic_len);

    if (memcmp(m, &decrypted[a_len], m_len) == 0) {
        printf("decrypted: succeed\n");
    } else {
        printf("decrypted: failed\n");
        while (1);
    }

    //dump(m, m_len);

    printf("mic_dec:\n");
    dump(mic_dec, mic_len);

    printf("------------------------------------------------------\n");
    printf("crypto_test1: NEW API AES CCM STAR 128 encrypt & decrypt [%u]\n", m_len);
    printf("------------------------------------------------------\n");

    memset(mic_enc, 0, sizeof(mic_enc));
    memset(mic_dec, 0, sizeof(mic_dec));

    uint8_t input[m_len];
    uint8_t output[m_len];

    int ret = 0;

    // ------------------------- CCM STAR ENCRYPTION ------------------------

    vc_ccm_context enc_context;
    memset(&enc_context, 0, sizeof(vc_ccm_context));
    
    memcpy(enc_context.key, key, sizeof(key));
    enc_context.key_len = sizeof(key);
    enc_context.cipher_ctx.type = VC_CIPHER_AES_128_CCM;
    enc_context.cipher_ctx.mode = VC_MODE_CCM;
    enc_context.cipher_ctx.operation = VC_OPERATION_ENCRYPT;

    memset(input, 0, sizeof(input));
    memset(output, 0, sizeof(output));

    memcpy(input, m, sizeof(input));

    ret = vccrypt_ccm_star_encrypt_and_tag(&enc_context, sizeof(input),
                                           nonce, sizeof(nonce),
                                           a, a_len,
                                           input, output,
                                           mic_enc, mic_len);

    if (ret != 0) {
        printf("vccrypt_ccm_star_encrypt_and_tag: FAILED - ret: %d\n", ret);
        while (1);
    }

    if (memcmp(output, &encrypted[a_len], sizeof(output)) == 0) {
        printf("encrypted: succeed\n");
    } else {
        printf("encrypted: failed\n");
        while (1);
    }

    //dump(output, sizeof(output));

    printf("mic_enc:\n");
    dump(mic_enc, mic_len);

    // ------------------------- CCM STAR DECRYPTION ------------------------

    vc_ccm_context dec_context;
    memset(&dec_context, 0, sizeof(vc_ccm_context));

    memcpy(dec_context.key, key, sizeof(key));
    dec_context.key_len = sizeof(key);
    dec_context.cipher_ctx.type = VC_CIPHER_AES_128_CCM;
    dec_context.cipher_ctx.mode = VC_MODE_CCM;
    dec_context.cipher_ctx.operation = VC_OPERATION_DECRYPT;

    memcpy(input, output, sizeof(input));
    memset(output, 0, sizeof(output));

    ret = vccrypt_ccm_star_auth_decrypt(&dec_context, sizeof(input),
                                        nonce, sizeof(nonce),
                                        a, a_len,
                                        input, output,
                                        mic_dec, mic_len);

    if (ret != 0) {
        printf("vccrypt_ccm_star_auth_decrypt: FAILED - ret: %d\n", ret);
        while (1);
    }

    if (memcmp(output, &decrypted[a_len], m_len) == 0) {
        printf("decrypted: succeed\n");
    } else {
        printf("decrypted: failed\n");
        while (1);
    }

    //dump(output, sizeof(output));

    printf("mic_dec:\n");
    dump(mic_dec, mic_len);

    printf("------------------------------------------------------\n");
    printf("crypto_test1: NEW API AES CCM 128 encrypt & decrypt [%u]\n", m_len);
    printf("------------------------------------------------------\n");

    // ------------------------- CCM ENCRYPTION ------------------------

    memset(&enc_context, 0, sizeof(vc_ccm_context));
    
    memcpy(enc_context.key, key, sizeof(key));
    enc_context.key_len = sizeof(key);
    enc_context.cipher_ctx.type = VC_CIPHER_AES_128_CCM;
    enc_context.cipher_ctx.mode = VC_MODE_CCM;
    enc_context.cipher_ctx.operation = VC_OPERATION_ENCRYPT;

    memset(input, 0, sizeof(input));
    memset(output, 0, sizeof(output));

    memcpy(input, m, sizeof(input));

    ret = vccrypt_ccm_encrypt_and_tag(&enc_context, sizeof(input),
                                      nonce, sizeof(nonce),
                                      a, a_len,
                                      input, output,
                                      mic_enc, mic_len);

    if (ret != 0) {
        printf("vccrypt_ccm_encrypt_and_tag: FAILED - ret: %d\n", ret);
        while (1);
    }

    if (memcmp(output, &encrypted[a_len], sizeof(output)) == 0) {
        printf("encrypted: succeed\n");
    } else {
        printf("encrypted: failed\n");
        while (1);
    }

    //dump(output, sizeof(output));

    printf("mic_enc:\n");
    dump(mic_enc, mic_len);

    // ------------------------- CCM DECRYPTION ------------------------

    memset(&dec_context, 0, sizeof(vc_ccm_context));

    memcpy(dec_context.key, key, sizeof(key));
    dec_context.key_len = sizeof(key);
    dec_context.cipher_ctx.type = VC_CIPHER_AES_128_CCM;
    dec_context.cipher_ctx.mode = VC_MODE_CCM;
    dec_context.cipher_ctx.operation = VC_OPERATION_DECRYPT;

    memcpy(input, output, sizeof(input));
    memset(output, 0, sizeof(output));

    ret = vccrypt_ccm_auth_decrypt(&dec_context, sizeof(input),
                                   nonce, sizeof(nonce),
                                   a, a_len,
                                   input, output,
                                   mic_dec, mic_len);

    if (ret != 0) {
        printf("vccrypt_ccm_auth_decrypt: FAILED - ret: %d\n", ret);
        while (1);
    }

    if (memcmp(output, &decrypted[a_len], m_len) == 0) {
        printf("decrypted: succeed\n");
    } else {
        printf("decrypted: failed\n");
        while (1);
    }

    //dump(output, sizeof(output));

    printf("mic_dec:\n");
    dump(mic_dec, mic_len);
#endif
}

static void crypto_test2(void)
{
#if TEST2
    printf("------------------------------------------------------\n");
    printf("crypto_test2: auth only\n");
    printf("------------------------------------------------------\n");

    uint8_t key2[] = {
        0x8e, 0x59, 0xaf, 0x41, 0x4a, 0xd6, 0xf6, 0xa3, 0x42, 0xce, 0xe5, 0xbf, 0x0e, 0x96, 0x56, 0x8b
    };

    uint8_t nonce2[] = {
        0xff, 0xff, 0xff, 0xff, 0xf4, 0x59, 0xec, 0x53, 0x00, 0x00, 0x00, 0x01, 0x06
    };

    uint8_t a2[] = {
        0x4a, 0xee, 0x28, 0xd2, 0x04, 0x00, 0xfe, 0xff,
        0x00, 0x00, 0x00, 0x53, 0xec, 0x59, 0xf4, 0xff,
        0xff, 0xff, 0xff, 0x0e, 0x01, 0x00, 0x00, 0x00,
        0x02, 0x05, 0x15, 0x01, 0x05, 0xb8, 0x61, 0x01,
        0x02, 0x15, 0x04, 0xa8, 0x06, 0x15, 0x02, 0xbb,
        0x04, 0xbe, 0x01, 0x00
    };

    uint8_t mic2[8];
    uint8_t mic2_auth[8] = {
        0xbe, 0x5d, 0xb3, 0xbf, 0x9c, 0x82, 0x46, 0x62
    };

    vccrypt_aes_ccm_encrypt(1, key2, nonce2, NULL, 0, a2, sizeof(a2), mic2, sizeof(mic2));

    if (memcmp(mic2, mic2_auth, sizeof(mic2)) == 0) {
        printf("mic: ok\n");
    } else {
        printf("mic: failed\n");
    }

    dump(mic2, sizeof(mic2));

    printf("------------------------------------------------------\n");
    printf("crypto_test2: NEW API AES CBC STAR 128 auth only\n");
    printf("------------------------------------------------------\n");

    // Note: for auth only must use CBC cipher mode

    int ret = 0;

    memset(mic2, 0, sizeof(mic2));

    vc_ccm_context context;
    memset(&context, 0, sizeof(vc_ccm_context));

    memcpy(context.key, key2, sizeof(key2));
    context.key_len = sizeof(key2);
    context.cipher_ctx.type = VC_CIPHER_AES_128_CBC;
    context.cipher_ctx.mode = VC_MODE_CBC;
    context.cipher_ctx.operation = VC_OPERATION_ENCRYPT;

    uint8_t tag[8];
    memset(tag, 0, sizeof(tag));

    ret = vccrypt_ccm_star_encrypt_and_tag(&context, 0,
                                           nonce2, sizeof(nonce2),
                                           a2, sizeof(a2),
                                           NULL, NULL,
                                           tag, sizeof(tag));

    if (ret != 0) {
        printf("vccrypt_ccm_star_encrypt_and_tag: FAILED - ret: %d", ret);
        while (1);
    }

    if (memcmp(tag, mic2_auth, sizeof(tag)) == 0) {
        printf("mic: ok\n");
    } else {
        printf("mic: failed\n");
        while (1);
    }

    dump(tag, sizeof(tag));
#endif
}

static void crypto_test3(void)
{
#if TEST3
    uint8_t key3[] = {
        0x8b, 0xcf, 0x9c, 0x28, 0xde, 0x3e, 0x46, 0x60, 0xbd, 0x3a, 0x65, 0x7c, 0x47, 0xce, 0x2c, 0xe3
    };

    uint8_t nonce3[] = {
        0x00, 0x00, 0x03, 0xff, 0xfe, 0x75, 0x02, 0x89, 0x00, 0x00, 0x00, 0x5f, 0x06
    };

    uint8_t a3[] = {
        0x69, 0xee, 0xa6, 0x8c, 0x0d, 0x00, 0x00, 0x00,
        0x23, 0x18, 0x00, 0x89, 0x02, 0x75, 0xfe, 0xff,
        0x03, 0x00, 0x00, 0x0e, 0x5f, 0x00, 0x00, 0x00,
        0x01, 0x05, 0x15, 0x01, 0x04, 0x13, 0x79, 0x0b,
        0x06, 0x15, 0x02, 0x4b, 0x02, 0x91, 0x01, 0x00,
        0x00, 0x3f
    };

    uint8_t m3[] = {
        
    };

    uint8_t m3_enc[] = {
        
    };

    uint8_t mic3[8];
    uint8_t mic3_auth[8] = {
        0xd3, 0x96, 0x6f, 0x35, 0x30, 0x9a, 0x2c, 0x3c
    };

    uint8_t input[sizeof(m3)];
    uint8_t output[sizeof(m3)];

    memset(input, 0, sizeof(input));
    memset(output, 0, sizeof(output));

    printf("------------------------------------------------------\n");
    printf("crypto_test3: encrypt [%u]\n", sizeof(m3));
    printf("------------------------------------------------------\n");

    memcpy(input, m3, sizeof(input));

    vccrypt_aes_ccm_encrypt(1, key3, nonce3, input, sizeof(input), a3, sizeof(a3), mic3, sizeof(mic3));

    if (memcmp(input, m3_enc, sizeof(input)) == 0) {
        printf("encrypt: ok\n");
    } else {
        printf("encrypt: failed\n");
        while (1);
    }

    //dump(input, sizeof(input));

    if (memcmp(mic3, mic3_auth, sizeof(mic3)) == 0) {
        printf("mic: ok\n");
    } else {
        printf("mic: failed\n");
        while (1);
    }

    dump(mic3, sizeof(mic3));

    printf("------------------------------------------------------\n");
    printf("crypto_test3: NEW API CCM STAR 128 encrypt [%u]\n", sizeof(m3));
    printf("------------------------------------------------------\n");

    int ret = 0;

    uint8_t tag[8];
    memset(tag, 0, sizeof(tag));

    vc_ccm_context context;
    memset(&context, 0, sizeof(vc_ccm_context));

    memcpy(context.key, key3, sizeof(key3));
    context.key_len = sizeof(key3);
    context.cipher_ctx.type = VC_CIPHER_AES_128_CCM;
    context.cipher_ctx.mode = VC_MODE_CCM;
    context.cipher_ctx.operation = VC_OPERATION_ENCRYPT;

    memset(input, 0, sizeof(input));
    memcpy(output, 0, sizeof(output));

    memcpy(input, m3, sizeof(input));

    ret = vccrypt_ccm_star_encrypt_and_tag(&context, sizeof(input),
                                           nonce3, sizeof(nonce3),
                                           a3, sizeof(a3),
                                           input, output,
                                           tag, sizeof(tag));

    if (ret != 0) {
        printf("vccrypt_ccm_star_encrypt_and_tag: FAILED - ret: %d", ret);
        while (1);
    }

    if (memcmp(output, m3_enc, sizeof(output)) == 0) {
        printf("encrypt: ok\n");
    } else {
        printf("encrypt: failed\n");
        while (1);
    }

    //dump(output, sizeof(output));

    if (memcmp(tag, mic3_auth, sizeof(tag)) == 0) {
        printf("mic: ok\n");
    } else {
        printf("mic: failed\n");
        while (1);
    }

    dump(tag, sizeof(tag));
#endif
}

static void dump(uint8_t *buf, uint32_t len)
{
    for (uint32_t i = 0; i < len; i++) {
        if ((i % 8) == 0) {
            if (i != 0) {
                printf("\n");
            }
        }
        printf("%02X ", buf[i]);
    }
    printf("\n");
}
